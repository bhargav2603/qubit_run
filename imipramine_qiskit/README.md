...
rebuilt to run locally with Qiskit and Qiskit Aer instead of Fujitsu QARP, MPI
and Qulacs on an A64FX cluster.
...
**byte-identical** to the QARP folder's and `hamiltonian.py` differs only in
comments — `tests/test_core.py` fails if that stops being true — so a
disagreement between the two backends is a backend result, not a different
molecule.
...
Produced on Colab (Linux, Python 3.12.13, qiskit 1.0.2, pyscf 2.14.0). Full JSON,
receipts and figures are archived under `Results/imipramine_results/` and
`Results/imipramine_visualizations/`.

| Run | Pool | Operators | Error vs. CASCI | ⟨S²⟩ | 2q gates | Depth | Time | Status |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `adapt_uccgsd` | 435 ops | 58 | **+0.0173 mHa** | 1.1 × 10⁻⁶ | 2,988 | 5,296 | 133 s | verified |
| `adapt_uccsd` | 63 ops | 31 (budget exhausted) | +0.8917 mHa | 2.8 × 10⁻³ | 2,515 | 3,424 | 52 s | **spin contaminated** |
...
optimized energy to 8.1 × 10⁻¹⁰ Ha with exact (non-Trotter) synthesis.
