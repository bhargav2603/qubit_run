...
QARP_FOLDER = ROOT.parent / "imipramine_cas6e6o"
...
def _dataclass_field_names(path: Path, class_name: str) -> list[str] | None:
    """Annotated field names of one dataclass, read without importing the module.

    The sibling QARP folder defines modules with the same names as this one, so
    importing it here would collide. Parsing is also the honest tool: what
    matters is the declared field set, because that -- and nothing else -- is
    what `spec_fingerprint` hashes.
    """
    if not path.is_file():
        return None
...
class SiblingWorkflowTests(unittest.TestCase):
    """The QARP and Qiskit folders must be running the same chemistry."""

    def test_molecule_is_byte_identical_to_the_qarp_workflow(self) -> None:
        sibling = QARP_FOLDER / "molecule.py"
        if not sibling.is_file():
            self.skipTest("the QARP workflow is not present next to this one")
        self.assertEqual(
            (ROOT / "molecule.py").read_bytes(),
            sibling.read_bytes(),
            "the two backends would be solving different molecules",
        )

    def test_validation_is_byte_identical_to_the_qarp_workflow(self) -> None:
        sibling = QARP_FOLDER / "validation.py"
        if not sibling.is_file():
            self.skipTest("the QARP workflow is not present next to this one")
        self.assertEqual(
            (ROOT / "validation.py").read_bytes(),
            sibling.read_bytes(),
            "the two backends would be validating to different standards",
        )

    def test_specification_schema_matches_the_qarp_workflow(self) -> None:
        """Same fields, so the same molecule literal hashes to the same digest."""
        sibling = _dataclass_field_names(QARP_FOLDER / "hamiltonian.py", "MoleculeSpec")
        if sibling is None:
            self.skipTest("the QARP workflow is not present next to this one")
        self.assertEqual(
            _dataclass_field_names(ROOT / "hamiltonian.py", "MoleculeSpec"), sibling
        )
