...
by construction rather than by penalty. The emitted Qiskit circuit, 2{,}988
two-qubit gates at depth 5{,}296, reproduces the optimised energy to
$8.1\times10^{-10}\Ha$ with exact rather than Trotterised synthesis.
...
\subsection{Relationship to the QARP implementation}

This workflow is a backend port. \code{molecule.py} and \code{validation.py} are
byte-identical to those of the Fujitsu QARP / Qulacs implementation in
\code{../imipramine\_cas6e6o/}, and \code{hamiltonian.py} differs only in its
docstring and two comments; \code{tests/test\_core.py} fails if that ever stops
being true. The consequence is that a disagreement between the two backends is a
backend result and not a different molecule --- which is the only condition
under which comparing them would mean anything.
...
\midrule
\code{uccgsd} (435) & $58$ & $\mathbf{0.0173}$ & $1.10\times10^{-6}$ & $2{,}988$ & $5{,}296$ & verified \\
\code{uccsd} (63)   & $31$ & $0.8917$          & $2.76\times10^{-3}$ & $2{,}515$ & $3{,}424$ & \textbf{spin contaminated} \\
\bottomrule
\end{tabular}
\end{table}
...
Pool build time               & $37.4$ s         & $14.0$ s \\
Optimisation time             & $116.3$ s        & $43.2$ s \\
Total wall time               & $133.0$ s        & $52.4$ s \\
Circuit energy residual       & $8.11\times10^{-10}\Ha$ & $6.78\times10^{-10}\Ha$ \\
...
& \code{uccgsd} & \code{uccsd} \\
\midrule
Two-qubit gates    & $2{,}988$ & $2{,}515$ \\
Single-qubit gates & $4{,}674$ & $1{,}960$ \\
Depth              & $5{,}296$ & $3{,}424$ \\
...
\midrule
ADAPT \code{uccgsd} & $-842.0180246890$ & $0.017$ & $\mathbf{133.0}$ \\
\bottomrule
\end{tabular}
\end{table}
...
ADAPT-VQE reaches $0.017\mHa$ in $133$ seconds. CAS(6e,6o) holds 400
...
\item \textbf{A comparison against the QARP/Qulacs backend.} The chemistry is
  pinned identical by test, which is the precondition for such a comparison, but
  the two backends were not run under matched settings here.
...
\item ADAPT-VQE with a generalized pool reaches $0.0173\mHa$ of exact
  ...
  qubits, in $133$ seconds. It recovers $99.92\%$ of the correlation energy
  available there, its particle-number leakage is exactly zero, and no penalty
  term was used.

\item The emitted circuit --- 2{,}988 two-qubit gates, depth 5{,}296 ---
  reproduces that energy to $8.1\times10^{-10}\Ha$ under exact synthesis, so the
...
Physics source          & \code{c6cd75127aac39a1} \\
Chemistry workflow      & \code{c4682e8536f67d69} \\
ADAPT workflow          & \code{afdc411eda4593dd} \\
...
Linux 6.6.122 (Google Colab), Python 3.12.13. qiskit 1.0.2, qiskit-aer 0.14.2,
OpenFermion 1.8.1, PySCF 2.14.0, NumPy 1.26.4, SciPy 1.17.1.

The workflow also runs on Windows 11 / Python 3.12.13 with qiskit 2.5.2,
qiskit-aer 0.17.2, OpenFermion 1.8.1, NumPy 2.5.2, SciPy 1.18.0 --- every
command except \code{prepare} and \code{classical}, which require PySCF and
therefore a platform for which it publishes a wheel.
