...
`tests/test_core.py` also pins the specification fingerprint, so the two active
spaces can never be confused for one another and a cache built for one cannot
