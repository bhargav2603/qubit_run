...
This is the only module that touches a simulator. The chemistry lives in
`molecule.py`, `hamiltonian.py` and `validation.py`, none of which import
Qiskit, so the execution layer can be edited without being able to alter -- or
invalidate the proof of -- a Hamiltonian that was built and validated
elsewhere. `physics_fingerprint` in `hamiltonian.py` enforces that split.

Four properties this layer guarantees rather than assumes:
...
* **Observables at the optimum are always available**, so the physical energy,
  ...
  A result that reported an energy without being able to price the symmetry it
  broke would not be checkable, and that failure mode cannot arise here.
* **The sector energy bound is still checked.** Validation proves the exact
  ground energy of the six-electron sector; a converged state below it by more
  than its own measured leak is a contradiction, not a better number, and is
  reported as one.
...
def build_ansatz(n_qubits: int, layers: int) -> Any:
    ...
    `real_amplitudes` is RY rotations plus a linear CX ladder: the standard
