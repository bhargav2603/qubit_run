...
§6, *Claims not supported by these data*, lists the five claims declined and the
...
Both runs carry identical cache, specification and physics digests
(`70982acc869f5f49`), so they solved the same operator — which is the
precondition for comparing them at all.

Two machines, both recorded in every result file. The Hamiltonian and the
classical ladder were built on Colab (Linux, Python 3.12.13, pyscf 2.14.0) —
the only stage needing PySCF. Both ADAPT runs and the validation receipt were
produced on Windows 11 / Python 3.13.7 / qiskit 2.3.1. The Colab runs under
qiskit 1.0.2 give the same energies to the digits reported; only transpiled gate
counts and wall times differ.
