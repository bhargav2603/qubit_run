...
def _qarp_lih_keywords() -> dict[str, object] | None:
    """Read `LIH = MoleculeSpec(...)` out of the QARP workflow without importing it.

    `../lih/chemistry.py` imports PySCF at module scope, so it cannot be
    exec'd on Windows -- and exec'ing a sibling module to read one constant is
    the wrong tool anyway. Parsing the literal keeps the cross-check available
    everywhere.
    """
    sibling = ROOT.parent / "lih" / "chemistry.py"
    if not sibling.is_file():
        return None
    tree = ast.parse(sibling.read_text(encoding="utf-8"), str(sibling))
    for node in tree.body:
        if not isinstance(node, ast.Assign):
            continue
        targets = [t.id for t in node.targets if isinstance(t, ast.Name)]
        if "LIH" not in targets or not isinstance(node.value, ast.Call):
            continue
        return {
...
class MoleculeSpecificationTests(unittest.TestCase):
    ...
    def test_specification_matches_the_qarp_workflow(self) -> None:
        """The two backends must be solving the same molecule, not a similar one."""
        keywords = _qarp_lih_keywords()
        if keywords is None:
            self.skipTest("the QARP workflow is not present next to this one")
        self.assertEqual(keywords["basis"], LIH_FROZEN_CORE.basis)
        self.assertEqual(
            [(symbol, tuple(coords)) for symbol, coords in keywords["atoms"]],
            [(symbol, tuple(coords)) for symbol, coords in LIH_FROZEN_CORE.atoms],
        )
        # Absent keywords are the MoleculeSpec defaults over there: neutral singlet.
        self.assertEqual(keywords.get("charge", 0), LIH_FROZEN_CORE.charge)
        self.assertEqual(keywords.get("spin", 0), LIH_FROZEN_CORE.spin)
