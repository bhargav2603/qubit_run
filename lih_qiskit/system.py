...
The molecule does not change because the simulator does: the geometry and basis
here are the ones `../lih/chemistry.py` fed to QARP on the FX700, and
`tests/test_core.py` reads that file to prove it. Only the active space is
...
_GEOMETRY_SOURCE = (
    ...
    "(Huber & Herzberg, via NIST CCCBDB), rounded to 1.595. Identical to the "
    "geometry in ../lih/chemistry.py, which is what makes the QARP/Qulacs and "
    "Qiskit Aer results comparable."
)
