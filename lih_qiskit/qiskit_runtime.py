...
This replaces `../lih/lih_vqe.py`, which drove the same LiH problem through
QARP/Qulacs on a Fujitsu FX700. The chemistry is untouched; only the simulator
changes. The lesson that script was written around survives verbatim: starting
the ansatz at the |0...0> *vacuum* makes the optimizer spend its budget
climbing out of a particle-number penalty and converge to an energy above
Hartree-Fock. The Hartree-Fock frame rotation below is the fix.

Three things are deliberately better here than in the QARP version, because
...
def build_ansatz(
    ...
    `real-amplitudes` is RY rotations plus a linear CX ladder, which is exactly
