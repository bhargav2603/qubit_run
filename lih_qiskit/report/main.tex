...
to $1.595\Ang$, in vacuum, closed shell, in the STO-3G basis. This is
byte-identical to the geometry used by the QARP/Qulacs implementation of the
same system in \code{../lih}, and \code{tests/test\_core.py} parses that file and
...
\item \textbf{A comparison against the QARP/Qulacs implementation in
  \code{../lih}.} The geometry and basis are pinned equal by a test, which is
  what would make such a comparison meaningful, but the runs were not executed
  under matched optimiser settings and are not compared here.
