...
class MoleculeSpecificationTests(unittest.TestCase):
    ...
    def test_geometry_matches_the_qarp_workflow(self) -> None:
        """Same molecule as the cluster workflow, even though the model differs.

        The full spec hashes are *expected* to differ now -- this workflow uses
        MP2 natural orbitals and PCM where the QARP one uses canonical vacuum
        orbitals -- so only the geometry is compared.
        """
        sibling = ROOT.parent / "acrylamide_thiolate" / "system.py"
        if not sibling.is_file():
            self.skipTest("the QARP workflow is not present next to this one")
        namespace: dict[str, object] = {}
        exec(compile(sibling.read_text(encoding="utf-8"), str(sibling), "exec"), namespace)  # noqa: S102
        other = namespace["ACRYLAMIDE_THIOLATE"]
        self.assertEqual(
            [(s, tuple(round(v, 6) for v in c)) for s, c in other.atoms],  # type: ignore[attr-defined]
            [(s, tuple(round(v, 6) for v in c)) for s, c in make_spec(geometry="ts").atoms],
        )
